...
class Network_item(object):  #clase de la que heredan los elementos de la red hub y host
    def __init__(self,name,p_number):  #se inicializa con los parametros nombre y cantidad de puertos
        ...
        self.send_state = [-1 fo i in range(p_number)]
...
class Switch(Network_item):
    def __init__(self,name,p_number):
        ...
        self.MAC_table = {}
        self.aging_time = 600000  #agin_time, 10 min(si un host esta sin enviar mas d 10 min se quita d ese puerto)
        self.buf = [{} for i in range(p_number + 1)] #un bufer por cada puerto del switch, para guardar el frame 
        self.buf_index = [0 for i in range(p_number + 1)]
        self.buf_index_send = [0 for i in range(p_number + 1)]
    ...
    def transfer_frame_to_send(port):
        ...
        if dest_mac != "FFFF" or dest_mac not in self.MAC_table:
...
class Host(Network_item):  #clase correspondiente a las computadoras
    ...
    def manage_collision(self):   ##funcion pa manejar las colisiones
...
class Network(object):
    ...
    def query_parse(self,query):  #analizar la query y ejecutar la funcion q corresponda
        ...
        if name_query == "create":
            item = query[2]   #elemento a crear: hub o host
    ...
    def add_hub(self,name,n_ports): #funcion para agregar un hub a la red, con nombre name y n_ports puertos
        ...
        self.connections[hub_] = [None for i in range(n_ports)]  #las conexionse se inicializan en None
    ...
    def check_local_network(self):  #funcion para chequear el estado de la red
        connected_devices = self.dfs()  #se realiza un dfs para ver los dispositivos conectados entre si en la red
        hosts_attempting_to_send = self.detect_collisions(connected_devices)  #esta funcion devuelve las computadoras que 
                                                                            #enviaran datos en este milisegundo
        if not hosts_attempting_to_send:  #se chequea si ninguna computadora enviara en este momento
            for host in self.hosts:   #por cada computadora se chequea si tiene data pendiente de enviar para saber que no
                                    #puede terminarse la ejecucion
                if host.data:
                    return 0
            return 1
        self.dfs_update_states(hosts_attempting_to_send)  #se realiza un dfs para actualizar el estado de cada dispositivo
        return 0
    
    def dfs(self):  #dfs para chequear los dispositivos interconectados en la red
        visited = {value : False for value in self.dict_name_to_item.values()}  #diccionario que por cada dispositivo tendra True
                                                                                #si ya fue visitado en el dfs, False si no
        
        connected_devices = {value : -1 for value in self.dict_name_to_item.values()} #por cada elemento tendra el valor de la componente 
                                                                                        #conexa que le corresponde en la red
        count_cc = 1
        for i in self.hosts: #se inicia el dfs partiendo de cada computadora que no se haya visitado
            if not visited[i]:
    ...
    def dfs_visit(self, i, connected_devices, visited, count_cc): #funcion para el dfs_visit
        visited[i] = True
        connected_devices[i] = count_cc

        for item in self.connections[i]:
            if not item == None and not visited[item[0]]:
                self.dfs_visit(item[0],connected_devices,visited,count_cc)

    def detect_collisions(self, connected_devices):  #funcion para detectar las posibles colisiones en la red
        xor = [0 for i in range(len(connected_devices))]  #por cada componente conexa se tendra el xor de los valores que
        ...
        for item in self.hosts:  #se chequea por cada computadora si tiene que enviar en este milisegundo
            if isinstance(item,Host) and item.time_to_send == self.time:
                xor[connected_devices[item]] ^= int(item.data[item.bit_to_send_pos]) 
        ...
        for item in hosts_attempting_to_send: 
            if int(item.data[item.bit_to_send_pos]) != xor[connected_devices[item]]: #detecta colision si el bit a enviar es distinto del xor 
                                                                                    #obtenido para su componente conexa
                item.manage_collision()  #funcion para que la computadora maneje la colision
                items_to_remove.append(item)

        for item in items_to_remove:  #se quitan de la lista de computadorasque van a enviar informacion 
                                        #a todas las que tienen colision
            hosts_attempting_to_send.remove(item)
        return hosts_attempting_to_send

    def dfs_update_states(self, hosts_attempting_to_send):  #funcion para actualizar el estado de cada dispositivo usando dfs
        visited = {value : False for value in self.dict_name_to_item.values()}
        for i in hosts_attempting_to_send:  #se comienza el dfs por cada computadora que tiene que enviar datos en este milisegundo
        ...
        for port in range(len(self.connections[i])):  #se revisa cada puerto del dispositivo
            ...
            if not item == None and (not visited[item[0]] or (isinstance(item[0],Hub) and isinstance(i,Host) and (item[0].p_sending[item[1]] == 0 or item[0].p_sending[item[1]] == -1))):
                ...
                if (isinstance(item[0], Host) and item[0].p_sending[0] != 2) or isinstance(item[0], Hub): #chequea si item[0] es una
                                                                                    #computadora que no esta colicionando o un hub y se 
                                                                                    #actualiza su esta a 1(recibiendo)
                    item[0].p_sending[item[1]] = 1 
                item[0].flow_data = i.flow_data
                if not visited[item[0]] and not isinstance(item[0],Host):  #se llama dfs si no se ha visitado el
